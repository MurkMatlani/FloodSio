...
const RiderModal = ({ isVisible = false ,onClose,onRiderSelect}) => {
    const { allRiders } = React.useContext(AdminContext);
    const Item = ({ item, backgroundColor, textColor }) => (
        <TouchableOpacity
        onPress={()=>{
            onRiderSelect(item._id);
        }}
                ...
                <View style={styles.centeredView}>
                    <View style={styles.modalView}>
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                            <Text style={styles.modalText}>All Riders</Text>
                            <AntDesign onPress={onClose} name="close" size={20} color="#111" />
                        </View>
                        <View style={{ flex:1 }}>
                            {allRiders == null ? <ActivityIndicator /> :
                                <FlatList
                                    nestedScrollEnabled
                                    data={allRiders}
