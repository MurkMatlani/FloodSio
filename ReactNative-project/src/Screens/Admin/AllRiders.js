...
const AllRiders = ({onEdit}) => {
  const { allRiders ,getAllRiders} = React.useContext(AdminContext);
  ...
  React.useEffect(()=>{setFilteredDataSource(allRiders)},[allRiders])

  const searchFilterFunction = (text) => {
    if (text) {
      const newData = allRiders.filter(
        function (item) {
      ...
      setFilteredDataSource(allRiders);
      setSearch(text);
    }
  };
  ...
  return (
    <View style={{ width: '100%', height: '94%', backgroundColor: "#fff" }}>
      <Text style={{ color: "#111", paddingHorizontal: 10, paddingVertical: 20, fontSize: 15, fontWeight: '600' }}>All Riders</Text>
      ...
      <View style={{ flex:1}}>
        {filteredDataSource == null ? <ActivityIndicator /> :
          ...
          refreshControl={
            ...
            ListEmptyComponent={() => {
              return <View style={{ paddingHorizontal: 70, paddingVertical: 100 }}><Text style={{ color: "#777", fontSize: 20, fontWeight: '600' }}>Riders is Not Available</Text></View>
            }}
