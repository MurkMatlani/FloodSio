...
const AllVenders = ({ onEdit }) => {
  const { allVenders ,getAllVender} = React.useContext(AdminContext);

  const [search, setSearch] = useState('');
  const [filteredDataSource, setFilteredDataSource] = useState(null);
  React.useEffect(()=>{setFilteredDataSource(allVenders)},[allVenders])
 ...
 const onRefresh=async()=>{
  ...
  const searchFilterFunction = (text) => {
    if (text) {
      const newData = allVenders.filter(
        function (item) {
      ...
      setFilteredDataSource(allVenders);
      setSearch(text);
    }
  };
  ...
  return (
    <View style={{ width: '100%', height: "94%", backgroundColor: "#fff" }}>
      <Text style={{ color: "#111", paddingHorizontal: 10, paddingVertical: 20, fontSize: 15, fontWeight: '600' }}>All Venders</Text>
      ...
      <View style={{flex:1 }}>
        {filteredDataSource == null ? <ActivityIndicator /> :
          ...
          />}
            ...
            ListEmptyComponent={() => {
              return <View style={{ paddingHorizontal: 70, paddingVertical: 100 }}><Text style={{ color: "#777", fontSize: 20, fontWeight: '600' }}>Venders is Not Available</Text></View>
            }}
